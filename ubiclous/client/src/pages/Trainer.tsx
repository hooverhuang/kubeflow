...
import { getNamespaces, getTrainer, deleteTrainJob } from '../api';
import './Page.css';
...
export default function Trainer() {
  ...
  const handleDelete = (name: string) => {
    if (!confirm(`確定刪除 TrainJob「${name}」？`)) return;
    setDeleting(name);
    deleteTrainJob(ns, name)
      .then(() => load(ns))
  ...
  return (
    <div className="page">
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginBottom: '1.25rem' }}>
        <div>
          <h1 className="page-title">Trainer 任務</h1>
          <div className="page-desc-box">
            <p className="page-desc page-desc-em">
              在 ubicloud 中檢視與清理 Kubeflow 的 TrainJob。
            </p>
          </div>
        </div>
